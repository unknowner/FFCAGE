...
tools['General'].parsePage = function(_data) {
	...
	for(var i = 0, len = _names.length; i < len; i++) {
		...
		})).append($('<img class="cageAddFavourite" src="resources://jid0-un8bmbr60nostzundr1tvlgn4hy-at-jetpack-cage-castle-age-game-enhancer-data/img/fav.png" alt="' + _e.name + '" />').hover(tools['General'].hoverAddIn, tools['General'].hoverAddOut).click(tools['General'].clickAdd)));
		if(_e.charge) {
...
tools['General'].clickAdd = function() {
	console.log('Add fav');
	if(tools['General'].runtime.favourites == null) {
		tools['General'].runtime.favourites = [];
	}
...
tools['General'].hoverAddIn = function() {
	...
	$(this).attr('src', 'resources://jid0-un8bmbr60nostzundr1tvlgn4hy-at-jetpack-cage-castle-age-game-enhancer-data/img/favadd.png');
};
tools['General'].hoverAddOut = function() {
	$('#cageSelectorInfo').html('');
	$(this).attr('src', 'resources://jid0-un8bmbr60nostzundr1tvlgn4hy-at-jetpack-cage-castle-age-game-enhancer-data/img/fav.png');
};
tools['General'].hoverRemoveIn = function() {
	$('#cageSelectorInfo').html('Remove ' + $(this).attr('alt') + ' from favourites');
	$(this).attr('src', 'resources://jid0-un8bmbr60nostzundr1tvlgn4hy-at-jetpack-cage-castle-age-game-enhancer-data/img/favdel.png');
};
