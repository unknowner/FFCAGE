...
tools.General.parsePage = function(_data) {
	...
	for(var i = 0, len = _names.length; i < len; i++) {
		...
		})).append($('<img class="cageAddFavourite" src="' + 'resource://cage-castle-age-game-enhancer-data/img/fav.png' + '" alt="' + _e.name + '" />').hover(tools.General.hoverAddIn, tools.General.hoverAddOut).click(tools.General.clickAdd)));
		if(_e.charge) {
...
tools.General.hoverAddIn = function() {
	...
	$(this).attr('src', 'resource://cage-castle-age-game-enhancer-data/img/favadd.png');
};
tools.General.hoverAddOut = function() {
	$('#cageSelectorInfo').html('');
	$(this).attr('src', 'resource://cage-castle-age-game-enhancer-data/img/fav.png');
};
tools.General.hoverRemoveIn = function() {
	$('#cageSelectorInfo').html('Remove ' + $(this).attr('alt') + ' from favourites');
	$(this).attr('src', 'resource://cage-castle-age-game-enhancer-data/img/favdel.png');
};
